...
func (g *Gateway) acceptConn(conn net.Conn) {
	...
	// add the peer
	g.addPeer(&peer{addr: addr, sess: muxado.Server(conn), inbound: true, version: remoteVersion})
	g.mu.Unlock(id)
...
func (g *Gateway) Connect(addr modules.NetAddress) error {
	...
	id = g.mu.Lock()
	g.addPeer(&peer{addr: addr, sess: muxado.Client(conn), inbound: false, version: remoteVersion})
	g.mu.Unlock(id)
